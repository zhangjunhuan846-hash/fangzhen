...
- 输入产物：`outputs/fitting/g6.1c/g6_1c_window_map_charge.csv`
- `dataset_role`: `benchmark`

## 扫描概况

- 窗口 237 个，其中可回放 230 个；0 个在整个扫描区间内连 1 mV 都推不到
- 未截断 16 个；其中带宽低于判据的 **3 个**

## 判定

| 参数 | 判定 | 依据协议 | 带宽 (dex) | 分辨率 (dex) | 证据 | 说明 |
|---|---|---|---|---|---|---|
| `Ds` | **identifiable** | `GITT-charge#t475` | 0.0988 | 0.050 | GITT | 未截断且 0.099 dex <= 判据 0.300 dex（水平 1 mV），可以引用数值（分辨率 0.0500 dex）；该窗口有 2 个探针点不可达（模型有效域边缘）；x0 = 0.0054；实测脉冲 \|dV\| = 596.6 mV |
...
```text
dlr_gitt/Hydra.0b
  Ds  : identifiable  (GITT-charge#t475)
