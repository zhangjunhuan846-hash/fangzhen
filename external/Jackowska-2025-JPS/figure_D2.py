"""
Plot a cost landscape for the areal 1C-discharge capacity and compare
...
with open(os.path.join(folder, "design_results", "parameters_after_D2p.pickle"), 'rb') as param_file:
...
class ArealCapacity(pybop.DesignCost):
    """
    Calculates the areal capacity of a battery cell, when applied to a
    normalised discharge from upper to lower voltage limits. The goal of maximising
    the areal capacity is achieved with self.minimising=False.

    The areal capacity [A.h.m-2] is calculated as

    .. math::
        \\frac{1}{3600 A} \\int_{t=0}^{t=T} I(t) \\mathrm{d}t

    where A is the electrode area, t is the time, T is the total time and I is the
    ...
    def compute(self, y, dy=None):
        ...
        areal_capacity = np.trapz(y["Current [A]"], dx=dt) / (
            3600
...
def make_prediction(param):
    ...
    Qarea = np.trapz(prediction["Current [A]"].data, dx=dt) / (
...
for i in range(len(param["Optimised electrode thickness [m]"])):
    x_optim.append(
        [
            ...
            param["Optimised nominal capacity [A.h]"][i],
        ]
    )
...
plt.figtext(0.94, 0.5, "Areal capacity (mA\,h\,cm$^{-2}$)", verticalalignment='center', rotation=270)
...
for x, label in zip([x_2mAh_cm2, x_4mAh_cm2, x_optim[-1]], ["2\,mA\,h\,cm$^{-2}$", "4\,mA\,h\,cm$^{-2}$", "Optimised"]):
    ...
    print("Areal capacity [mA.h.cm-2]:", Qarea / 10)
    if label != "Optimised":
...
save_path = os.path.join(folder, "design_results", "areal_capacity.pdf")
figc.savefig(save_path)
print("Plot saved at", save_path)

save_path = os.path.join(folder, "design_results", "areal_discharge.pdf")
