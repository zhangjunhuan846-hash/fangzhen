...
**① Control 二维图（SPM→SPM）**：两个倍率 argmin 都**精确落在真值**（分辨率 0.0107 dex）。
但谷是**脊不是碗** —— 伸长率 **3.2**，主轴与 iso-τ_d 夹角仅 **1.96°/3.16°**
→ **电压曲线主要约束 $\tau_d=R_p^2/D_s$**（$D_s$ 与 $R_p$ 部分退化，非完全退化）。

**② Mismatch（SPMe→SPM）**：

| 倍率 | $\theta=\{D_s\}$ RMS | $\theta=\{D_s,R_p\}$ RMS | 残差下降 | 双参数 $D_s$ | 双参数 $R_p$ |
|---|---|---|---|---|---|
| 0.1C | 1.656 mV | **0.480 mV** | **−71.0 %** | 2.63e-14（**+1215 %**） | 2.000e-05（**撞 20 µm 上界**） |
| 0.5C | 7.011 mV | **3.389 mV** | **−51.7 %** | 2.33e-14（**+1065 %**） | 1.886e-05（+261 %） |
...
±0.05 dex 的 J 抬升 = 0.1C **×3.447** / 0.5C **×3.024** / 1.5C **×1.466**。
→ 高倍率曲率**弱 2.4 倍但未塌陷**。**G5.2 的"杠杆作用丧失"表述过强，已更正。**
**教训：残差地板大只证明失配大，不证明目标函数平坦 —— 残差性质与曲率性质必须分别测量。**
（内部校验：单参数优化器偏差与 profile 最小值位置独立吻合：0.1C −25.0 % vs −0.10 dex 等。）

**边界**：仍合成；只测 0.1C/0.5C 两档；单失配方向；单电芯；伸长率 3.2 属"中等脊"，
严格说是**部分退化**。$R_p$ 上界（20 µm）是本次自设，0.1C 解撞界说明真解可能在外。
...
>
> 目前最重要的**科学**结论（G5.2 + G5.3，同一条链的两半）：
>
> ```text
> G5.2  错误模型下，"错"的参数优于真值；低倍率残差只有 1.66 mV、看不出异常
> G5.3  放开第二个参数不修复它 —— 因为曲线主要只约束 τ_d = R_p²/D_s，
>       而优化器正是沿那条浅方向逃逸（τ_d 仅变 12 %，D_s 却变 12 倍）
> ```
>
> 即：
