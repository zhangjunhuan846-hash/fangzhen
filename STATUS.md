...
G5 参数辨识 / 可信性      IN PROGRESS   — G5.0 PASS；G5.1 IN PROGRESS
  ...
  G5.4 多协议辨识性 + 模型一致性                PASS     — 2026-09-16（多协议无效，独立测量有效，见下）
  G5.5 真实实验 + cross-protocol 验证           NOT STARTED
...
### G5.4 多协议辨识性 + 模型一致性 — **PASS**（2026-09-16）
...
| 协议集 | $D_s$ | $R_p$ | equal-weighted RMS |
|---|---|---|---|
| P1 (0.1C) | +1215.4 % | 20.000 µm（**撞界**） | 0.480 mV |
| P2 (+0.5C) | +1086.3 % | 19.010 µm | 2.431 mV |
| P4 (+1C+1.5C) | +1364.1 % | 19.999 µm（**撞界**） | 25.975 mV |

**1 个协议加到 4 个，最优点几乎不动。** 但暴露了模型不一致 ——
**联合最优下 per-protocol 残差随倍率单调上升、跨度 25 倍**：
`C10 1.909 / C2 7.743 / 1C 19.567 / 1.5C 47.460 mV`。
且 **C10 自己的残差从 P1 的 0.480 涨到 P4 的 1.909** —— 联合拟合牺牲它去迁就别人，仍谁都满足不了。
...
`--group-c-only` 重跑了同一 JSON，**A+B 数字取自运行日志**（应分文件保存）。
