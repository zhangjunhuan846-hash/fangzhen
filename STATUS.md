...
G6 graphite‖Li 实验约束模型                   IN PROGRESS
  ...
  G6.1c GITT 激励地图（239 窗口）               **FAIL(M1/M2)** — 2026-09-16，见下
  G6.1b-2 3-region basis                        **暂缓**（G6.1c 已实测：**没有落脚点**）
...
### G6.1c GITT 激励地图 — **FAIL（M1 存在性 / M2 筛选性）**（2026-09-16）

完整报告 `docs/g6.1c_excitation_map.md`。
可复现：`python scripts/graphite/g6_1c_excitation_map.py --sweep discharge`
（**239 个放电窗口**，225 可用 × 43 探针点 = **10009 次仿真，24.5 min**；
两次独立完整运行**逐字节相同** —— 求解器可复现）
...
| 判据 | 结果 | 数字 |
|---|---|---|
| **M1 存在性**（$B\le0.30$ dex 且未截断） | **FAIL** | **0 个**（唯一的 1 个被截断） |
| M1b 同上，参考点 $a_0=-0.5$（冒烟后追加，secondary） | PASS | **4 个** |
| **M2 筛选性**（按实测 $\lvert dV_{\rm pulse}\rvert$ 的 top-10 与按 $B$ 的 top-10 重叠 $\ge5$） | **FAIL** | **4/10**，$\rho=-0.713$ |
| M3 单侧性（不对称量中位数 < 0） | PASS | **−0.0159** |
| M4 分辨率（中位 $\le0.10$ dex） | PASS | **0.050** dex |
| N1 负对照（零激励精确平坦） | PASS | max\|dV\| = **0**，两侧截断 |

**$B$ 的分布（225 个可用窗口）**：$\le0.30$：**1**｜0.3–0.5：2｜0.5–1.0：53｜
**$>1.0$（带跑到扫描边界外）：169（75 %）**；**中位数 = 2.000 dex = 整个扫描范围，完全没闭合**。

**五条结论**：
1. **绝大多数窗口对 $D_s$ 没有信息**（不是"少"，是"没有"）：75 % 在整个 $\pm1$ dex 内不闭合，
   中位数就是全范围宽度；另 5 个连"跨过 1 mV"都没发生。
2. **单侧性在大样本上成立**：**155/225（69 %）右侧截断**，不对称量中位数 −0.0159
   ⇒ G6.1b-1 在 6 个组合上看到的**是统计规律**；缺的那一半**统一在 $D_s$ 偏大侧**。
3. **"最好的窗口"正好在模型失效边界上**：`#t225`（$B=0.2149$，唯一看似合格）
   的 **41 个探针点里 19 个跑不到**；`#t224` 17 个；而**紧邻的 9 个窗口（#t231–#t239，$x_0\ge0.98$）
   根本回放不了**（`SolverError: Minimum voltage are non-positive at initial conditions`）。
   只有 6 个窗口（#t220–#t225）有不可达点，共 61 次运行。
   ⇒ **唯一合格的位置就是模型即将失效的位置，不可当作推荐。**
4. **实测脉冲幅度不是可靠的筛选量**（M2 FAIL）：$\rho=-0.713$；最可辨识的 `#t225`/`#t224`
   实测脉冲只有 12.4–12.6 mV，而最大的 `#t1`（279 mV）只排第 3。
   后验试过的"松弛/脉冲比"**符号还是反的（+0.287）**。
   只有**模型侧**的展开量强相关（$\rho=-0.914$）⇒ **选窗口必须跑模型**，
   "先筛数据再跑仿真"的省算力路线被否掉。
5. **9 个窗口无法回放**（$x_0\ge0.98$，撞最低电压事件）⇒ 可用率 **225/239 = 94 %**。

**推荐 protocol 子集：空集。** 规则（$B\le0.30$、未截断、按 $B$ 升序、
彼此 $x_0$ 至少隔 0.05）返回空 —— **这就是答案**，不是实现问题。

**三个跨阶段一致性核对**（两个独立入口，同一个估计器）：
`#t1` $a_0=0$ **0.35043** vs G6.1b-1 **0.3504**（逐位）；`#t1` $a_0=-0.5$ **0.13960** vs **0.1396**（逐位）；
`#t120` **0.89663** vs **0.897**（差 <0.001，来自 G6.1b-1 的 0.005 dex 细化）。

**决策（G6.1b-2 仍不进）**：导师的 "$rank(F)=1$" 在 $d=1$ 时是废话（$1\times1$ 必满秩），
它在 $d=3$ 时才是命题、且**本轮没实测**。但有一条**已被实测**的加强版：
**1 mV 带已经把"沿 $\phi_0=1$ 这一个方向"的分辨率测出来了 —— 中位 2.0 dex（测不出来）；
新增方向受同一份数据的信息量限制，每个系数有自己的带，交叉项只会让带更宽。**
⇒ **瓶颈在激励，不在参数化**；下一步是激励多样性（更长脉冲/更大倍率/多温度/避开模型失效区），
可用充电扫程做**复制检验**。

**级别依赖**：$B$ 锚在 1 mV；若局部二次，$B\propto\sqrt{\rm level}$，
换 5 mV 中位带约 0.89 dex，仍远大于 0.30 ⇒ **结论对水平取值不敏感**（量级判断，未实测）。

**边界**：只用放电扫程 239 窗口（充电扫程 237 未跑）；$B$ 是 model-to-model 量，
无噪声/无模型形式差异（**理想条件下最好情况**）；分母是 225 不是 239；
6 个窗口的带在被削过的定义域上量出且全部带 `truncated` 标记；
只有 SPM / 单电芯 / 26 °C / 150 s 脉冲；**不是对 Ecker2015 的验证**。
...
**G6 当前的真实卡点（2026-09-16，G6.1c 之后）**：不是能力、不是数据量，
而是**这份记录里没有能把 $D_s$ 定下来的激励**。
239 个放电窗口跑成地图（G6.1c）的结果：**在参考点上 0 个窗口合格**
（75 % 的窗口在 $\pm1$ dex 内连 1 mV 都不动），**69 % 单侧截断**，
而唯一"看起来合格"的窗口恰好压在模型失效边界上（41 个探针点 19 个跑不到，
紧邻 9 个根本回放不了的窗口）。
⇒ 下一步是**激励多样性**（更长脉冲 / 更大倍率 / 多温度 / 避开失效区）
与**充电扫程的复制检验**，**不是**加基函数。
