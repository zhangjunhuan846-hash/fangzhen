...
**参数来源矩阵**在 `docs/preregistration_annex_A_parameter_provenance.md`
（预注册附件 A，同样在看结果之前冻结）。每次 run 之前先对照它：模型的每一个参数值
都要能在那张表里找到**类别**（实测 / 实测派生 / 沿用药参集 / 允许辨识 / 不报）与出处。
它同时写死了三处**必须显式声明**的东西：**用哪个 `R`**、**截止电压取实验值还是
参数集值**（参数集自带的是 `0 V`，不是本实验的 `0.005 V`）、**终止原因**。
⚠️ 还有一条容易被忽略：`validate_series` **不检查料源**，所以
**一条 series 目录只能放同料源样品**（混料源不会被任何检查发现）。
