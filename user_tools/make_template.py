...
def build(path: Path) -> None:
    ...
    lines = [
        ...
        ("   chemistry        填 LFP / NMC / LCO / other（这一栏装的是正极体系）", False),
        ("   cell_configuration   full_cell = 全电池；half_cell = 半电池", False),
        ("   working_electrode    半电池才填：你的被测电极是 positive 还是 negative", False),
        ("   working_electrode_material  被测电极是什么材料（石墨填 graphite）", False),
        ("                    平台靠它决定该用哪个电压窗口；留空会跳过这项检查", False),
        ("                    例如石墨半电池的窗口是 0.005-1.5 V，填 [0.005, 15] 会被拦下", False),
        ...
        ("   protocol         仪器上的档位名，原样填（例如 CC_Cover5）", False),
        ("   protocol_type    协议类型，从下拉选（GCD / GITT / pOCV / ...）", False),
        ("                    填了 GITT：采样间断与脉冲时长不一致会直接判错误", False),
        ("                    因为 Ds 就是由脉冲时长算出来的", False),
        ("   pulse_duration_s / relax_duration_s  GITT 的脉冲与静置时长（秒）", False),
        ("                    填了平台会拿实测值核对；不填只给警告", False),
        ("", False),
